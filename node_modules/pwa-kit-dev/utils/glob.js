...
class Matcher {
  constructor(patterns) {
    ...
    const allPatterns = (patterns || []).map(pattern => new _minimatch.default.Minimatch(pattern, STANDARD_OPTIONS)).filter(pattern => !pattern.empty);
  ...
  matches(path) {
    if (path) {
      const positive = this._positivePatterns.some(pattern => pattern.match(path)); // A negative pattern.match returns true if the
