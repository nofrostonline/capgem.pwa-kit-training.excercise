...
const testFixtures = _path.default.resolve(__dirname, 'test_fixtures'); // Mocks methods on the DevServerFactory to skip setting
...
test('SSRServer proxying handles empty path', () => {
  ...
  const location = '/another/path'; // Create a mock response from the proxied backend

  const nockRedirect = (0, _nock.default)('https://test.proxy.com').get('/').reply(301, '', {
    Location: location
  });
  const options = opts(); // We expect the Express app to rewrite redirect responses

  const rewritten = `${options.protocol}://localhost:${options.port}/mobify/proxy/base${location}`;

  const app = NoWebpackDevServerFactory._createApp(options);

  return (0, _supertest.default)(app).get('/mobify/proxy/base/').expect(301).expect('Location', rewritten) // Expected to hit the backend
...
describe('DevServer proxying', () => {
  ...
  test('rewrites redirects', () => {
    ...
    return (0, _supertest.default)(app).get('/mobify/proxy/base/test/path').expect(301).expect('Location', rewritten) // Expected to hit the backend
    .then(() => expect(nockRedirect.isDone()).toBe(true));
  });
  test('rewrites headers', () => {
    ...
    return (0, _supertest.default)(app).get(path).set(outgoingHeaders).then(response => {
      ...
      expect(nockResponse.isDone()).toBe(true); // We expect a 200 (that nock returned)

      expect(response.status).toEqual(200); // We expect that we got a copy of the request headers

      expect(requestHeaders.length).toBe(1); // Verify that the request headers were rewritten

      const headers = requestHeaders[0];
      expect(headers.host).toEqual('test.proxy.com');
      expect(headers.origin).toEqual('https://test.proxy.com'); // Verify that the cookie and multi-value headers are
      // correctly preserved.

      expect(headers.cookie).toEqual('abc=123');
      const multi = headers['x-multi-value'];
      expect(multi).toEqual('abc, def'); // Verify that the response contains a Set-Cookie

      const setCookie = response.headers['set-cookie'];
      expect(setCookie.length).toBe(1);
      expect(setCookie[0]).toEqual('xyz=456'); // Verify that the x-proxy-request-url header is present in
      // the response
  ...
  test('filters headers', () => {
    ...
    const nockResponse = (0, _nock.default)('https://test.proxy.com').get('/test/path3').reply(200, function () {
      ...
      expect(headers['accept-encoding']).toEqual('gzip'); // This value is fixed
...
describe('DevServer persistent caching support', () => {
  ...
  test('Caching of compressed responses', () => {
    ...
    return Promise.resolve().then(() => (0, _supertest.default)(app).get(url)).then(res => app._requestMonitor._waitForResponses().then(() => res)).then(res => {
      ...
      key: keyFromURL(url),
      namespace
    })).then(entry => expect(entry.found).toBe(true)).then(() => (0, _supertest.default)(app).get(url)).then(res => app._requestMonitor._waitForResponses().then(() => res)).then(res => {
      expect(res.status).toEqual(200);
  ...
  test('Compressed responses are not re-compressed', () => {
    ...
    return (0, _supertest.default)(app).get(url).then(res => app._requestMonitor._waitForResponses().then(() => res)).then(res => {
      ...
      expect(res.headers['content-encoding']).toEqual('gzip');
      return res;
    }).then(res => app.applicationCache.get({
      key: keyFromURL(url),
      namespace
    }).then(entry => ({
      res,
      ...
      const uncompressed = _zlib.default.gunzipSync(entry.data);
...
describe('DevServer service worker', () => {
  ...
  const createApp = () => {
    const app = NoWebpackDevServerFactory._createApp(opts()); // This isn't ideal! We need a way to test the dev middleware
