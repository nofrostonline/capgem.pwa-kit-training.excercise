...
const DevServerMixin = {
  ...
  _addSDKInternalHandlers(app) {
    ...
    app.get('/__mrt/clear-browser-data', (_, res) => {
      ...
      console.log(''); // Note: this header value needs the double quotes.
  ...
  serveStaticFile(filePath, opts = {}) {
    return (req, res) => {
      req.app.__devMiddleware.waitUntilValid(() => {
        ...
        const serverCompilation = webpackStats.find( // static files are copied into bundle
        // in the server webpack config
        stat => stat.compilation.name === _configNames.SERVER).compilation;
        const {
          assetsInfo
        } = serverCompilation;
        const assetInfo = assetsInfo.get(filePath); // if the asset is not in the webpack bundle, then
...
const setLocalAssetHeaders = (res, assetPath) => {
  ...
  res.set('etag', mtime.getTime()); // We don't cache local bundle assets
