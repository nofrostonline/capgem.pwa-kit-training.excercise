...
const start = () => {
  ...
  const data = JSON.parse(document.getElementById('mobify-data').innerHTML); // Set all globals sent from the server on the window object.
  ...
  const locals = {}; // AppConfig.restore *must* come before getRoutes()

  AppConfig.restore(locals, window.__PRELOADED_STATE__.__STATE_MANAGEMENT_LIBRARY); // We need to tell the routeComponent HOC when the app is hydrating in order to
