  ...
  class WithReactQuery extends _fetchStrategy.FetchStrategy {
    ...
    static doInitAppState({
      ...
      return _asyncToGenerator(function* () {
        const queryClient = res.locals.__queryClient = res.locals.__queryClient || new _reactQuery.QueryClient(queryClientConfig); // Use `ssrPrepass` to collect all uses of `useQuery`.

        yield (0, _reactSsrPrepass.default)(appJSX);
        const queryCache = queryClient.getQueryCache();
        const queries = queryCache.getAll().filter(q => q.options.enabled !== false);
        yield Promise.all(queries.map(q => // If there's an error in this fetch, react-query will log the error
        q.fetch().catch(() => {// On our end, simply catch any error and move on to the next query
        })));
        return {
