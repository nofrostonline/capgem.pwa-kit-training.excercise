...
const RemoteServerFactory = {
  ...
  _configure(options) {
    ...
    (0, _ssrServer.setQuiet)(options.quiet || process.env.SSR_QUIET); // Set the protocol for the Express app listener - defaults to https on remote

    options.protocol = this._getProtocol(options); // Local dev server doesn't cache by default

    options.defaultCacheControl = this._getDefaultCacheControl(options); // Ensure this is a boolean, and is always true for a remote server.

    options.strictSSL = this._strictSSL(options); // This is the external HOSTNAME under which we are serving the page.
    ...
    options.devServerHostName = process.env.LISTEN_ADDRESS || `localhost:${options.port}`; // This is the ORIGIN under which we are serving the page.
  ...
  _logStartupMessage(options) {// Hook for the DevServer
  },
   ...
   */
  // eslint-disable-next-line no-unused-vars
  _setCompression(app) {// Let the CDN do it
  },
   ...
   */
  // eslint-disable-next-line no-unused-vars
  _setupMetricsFlushing(app) {// Hook for the dev-server
  },
  ...
  _createApp(options) {
    ...
    this._logStartupMessage(options); // To gain a small speed increase in the event that this
    ...
    const app = this._createExpressApp(options); // Do this first – we want compression applied to
    // everything when it's enabled at all.


    this._setCompression(app);

    this._setRequestId(app); // this._addEventContext(app)
    ...
    this._setupProxying(app, options); // Beyond this point, we know that this is not a proxy request
    ...
    const mixin = {
      ...
      get applicationCache() {
        if (!this._applicationCache) {
          ...
          const useLocalCache = !((0, _ssrServer.isRemote)() || bucket);
  ...
  _setupSSRRequestProcessorMiddleware(app) {
    const that = this; // Attach this middleware as early as possible. It does timing
    ...
    const processIncomingRequest = (req, res) => {
      const options = req.app.options; // If the request is for a proxy or bundle path, do nothing
      ...
      let updatedPath = req.path; // If there's an x-querystring header, use that as the definitive
      ...
      if (xQueryString) {
        updatedQuerystring = xQueryString; // Hide the header from any other code
      ...
      if (requestProcessor) {
        ...
        (0, _assert.default)(processed && 'path' in processed && 'querystring' in processed, 'Expected processRequest to return an object with ' + '"path" and "querystring" properties, ' + `but got ${JSON.stringify(processed, null, 2)}`); // Update the request.
      ...
      if (updatedQuerystring !== originalQuerystring) {
        ...
        parsed.search = updatedQuerystring ? `?${updatedQuerystring}` : ''; // Let Express re-parse the parameters
      ...
      parsed.pathname = updatedPath; // This will update the request's URL with the new path
      // and querystring.

      req.url = _url.default.format(parsed); // Get the request class and store it for general use. We
    ...
    const ssrRequestProcessorMiddleware = (req, res, next) => {
      ...
      locals.timer = new _ssrServer.PerformanceTimer(`req${locals.requestId}`);
      locals.originalUrl = req.originalUrl; // Track this response

      req.app._requestMonitor._responseStarted(res); // Start timing


      locals.timer.start('express-overall'); // If the path is /, we enforce that the only methods
      ...
      const afterResponse = () => {
        ...
        if (!locals.afterResponseCalled) {
          ...
          locals.afterResponseCalled = true; // Emit timing unless the request is for a proxy
          ...
          if (!req.originalUrl.startsWith('/mobify/')) {
            req.app.sendMetric('RequestTime', Date.now() - locals.requestStart, 'Milliseconds'); // We count 4xx and 5xx as errors, everything else is
          ...
          locals.timer.finish(); // Release reference to timer
      ...
      res.on('close', afterResponse); // Strip out API Gateway headers from the incoming request. We
  ...
  _setupCommonMiddleware(app, options) {
    app.use(prepNonProxyRequest); // Apply the SSR middleware to any subsequent routes that we expect users
  ...
  _validateConfiguration(options) {
    ...
    if (!_semver.default.satisfies(process.versions.node, // A string like '8.10.0'
    requiredNode)) {
      /* istanbul ignore next */
   ...
   * @private
   */
  _addStaticAssetServing() {// Handled by the CDN on remote
  },

  /**
   * @private
   */
  _addDevServerGarbageCollection() {// This is a hook for the dev-server. The remote-server
  ...
  serveServiceWorker(req, res) {
    const options = req.app.options; // We apply this cache-control to all responses (200 and 404)

    res.set(_constants.CACHE_CONTROL, // The CDN can cache for 24 hours. The browser may not cache
    // the file.
    's-maxage=86400, max-age=0');

    const workerFilePath = _path.default.join(options.buildDir, req.path); // If there is no file, send a 404
  ...
  _createHandler(app) {
    ...
    const handler = (event, context, callback) => {
      ...
      _awsServerlessExpress.default.proxy(server, event, // The incoming event
      ...
      (err, response) => {
        ...
        return app._requestMonitor._waitForResponses().then(() => app.metrics.flush()) // Now call the Lambda callback to complete the response
        .then(() => callback(err, (0, _ssrServer.processLambdaResponse)(response))) // DON'T add any then() handlers here, after the callback.
...
const prepNonProxyRequest = (req, res, next) => {
  const options = req.app.options; // Strip cookies from the request

  delete req.headers.cookie; // Set the Host header

  req.headers.host = options.appHostname; // Replace any Origin header
...
const applyPatches = once(options => {
  ...
  _https.default.get = (0, _ssrServer.outgoingRequestHook)(_https.default.get, options); // Patch the ExpressJS Response class's redirect function to suppress
  ...
  _express.default.response.redirect = function (status, url) {
    ...
    const address = this.location(workingUrl).get('Location'); // Send a minimal response with just a status and location
...
class RequestMonitor {
  ...
  _responseStarted(res) {
    ...
    const finish = () => this._responseFinished(res); // We hook both the 'finished' and 'close' events, so that
