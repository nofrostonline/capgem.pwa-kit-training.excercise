...
const generateCacheKey = (req, options = {}) => {
  let {
    pathname,
    query
  } = _url.default.parse(req.url); // remove the trailing slash
...
const storeResponseInCache = (req, res) => {
  ...
  const expiration = parseInt(caching.expiration || cacheControl['s-maxage'] || cacheControl['max-age'] || 7 * 24 * 3600); // Return a Promise that will be resolved when caching is complete.
...
const cacheResponseWhenDone = ({
  ...
  const locals = res.locals;
  const caching = locals.responseCaching; // If we have a key passed in, use that.
  // If we have a key already generated by getResponseFromCache, use that.
  // Otherwise generate the key from the request

  /* istanbul ignore next */

  caching.cacheKey = key || caching.cacheKey || generateCacheKey(req); // Save values that will be needed when we store the response

  caching.cacheNamespace = namespace;
  caching.expiration = expiration; // Set a flag that we use to detect a double call to end().
  ...
  res.end = (...params) => {
    ...
    caching.endCalled = true; // Handle any data writing that must be done before end()
    // is called.
...
const getResponseFromCache = ({
  ...
  /* istanbul ignore next */
  const locals = res.locals;
  const workingKey = key || generateCacheKey(req); // Save the key as the default for caching

  locals.responseCaching.cacheKey = workingKey; // Return a Promise that handles the asynchronous cache lookup
...
const respondFromBundle = ({
  ...
  const workingPath = path || req.path; // Validate redirect
  ...
  const assetPath = workingPath.startsWith('/') ? workingPath.slice(1) : workingPath; // This is the relative or absolute location of the asset via the
  // /mobify/bundle path
...
const getRuntime = () => {
  const runtime = (0, _ssrServer.isRemote)() ? _buildRemoteServer.RemoteServerFactory : // The dev server is for development only, and should not be deployed to production.
  ...
  eval('require').main.require('pwa-kit-dev/ssr/server/build-dev-server').DevServerFactory; // The runtime is a JavaScript object.
