...
describe('SSRServer operation', () => {
  ...
  test('SSRServer tracks responses', () => {
    ...
    expect(app._requestMonitor._pendingResponses.ids).toEqual([]); // If the promise doesn't resolve, this test will timeout
...
describe('SSRServer persistent caching', () => {
  ...
  testCases.forEach(testCase => test(testCase.name, () => {
    ...
    .then(response => app._requestMonitor._waitForResponses().then(() => response)) // Handle and verify the response
    .then(response => {
...
describe('generateCacheKey', () => {
  ...
  test('user agent affects key', () => {
    ...
    expect((0, _express.generateCacheKey)(request2)).not.toEqual(result1); // query string and device type is hashed
...
describe('getRuntime', () => {
  let originalEnv;
  let originalEval = global.eval; // Mock the DevSeverFactory via `eval` so we don't have to include it as a dev
