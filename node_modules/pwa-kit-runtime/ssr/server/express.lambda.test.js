...
describe('SSRServer Lambda integration', () => {
  ...
  lambdaTestCases.forEach(testCase => test(testCase.name, () => {
    ...
    server = srv; // Set up the mock proxy
    ...
    const metricSent = name => !!metrics.find(metric => !!metric.MetricData.find(data => data.MetricName === name)); // Set up a fake event and a fake context for the Lambda call
    ...
    .then(response => {
      ...
      expect(app.metrics.queueLength).toBe(0); // We're not asserting which metrics were sent, just
      // checking if any were sent. As of DESKTOP-434, every
      // request will send metrics.

      expect(!!metrics.length).toBe(true); // We check for some specific metrics here

      expect(metricSent('LambdaCreated')).toBe(true); // We expect a context property to have been set false

      expect(context.callbackWaitsForEmptyEventLoop).toBe(false); // Check the response
  ...
  test('Lambda integration strips rogue headers', () => {
    ...
    server = srv; // Set up a fake event and a fake context for the Lambda call
