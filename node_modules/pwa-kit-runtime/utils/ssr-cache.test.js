...
localRemoteTestCases.forEach(useLocalCache => {
  ...
  describe(nameBase, () => {
      ...
      describe(name, () => {
        ...
        test(`${name} delete existing`, () => {
          ...
          return testCache.get({
            ...
            key,
            namespace
          })).then(result => expect(result).toBeDefined()).then(() => testCache.delete({
            key,
            namespace
          })).then(() => testCache.get({
            key,
            namespace
          })).then(result => expect(result.found).toBe(false)).then(() => testCache.delete({
            key,
            namespace
          })).then(() => testCache.get({
            key,
        ...
        test(`${name} absolute expiration`, () => {
          ...
          return testCache.put({
            ...
            data: '12345',
            expiration
          }).then(() => sleep(500)).then(() => testCache.get({
            key,
            namespace
          })).then(result => {
            expect(result.found).toBe(false);
    ...
    test('sanitize returns expected value', () => {
      ...
      if (useLocalCache) {
        expect(testCache._sanitize(key, namespace).s3Key).toBe(`${namespace}${key}`.replace(/\//g, path.sep));
        expect(testCache._sanitize(key, namespace).safeNamespace).toBe('namespace/test/1/'.replace(/\//g, path.sep));
        expect(testCache._sanitize(key).s3Key).toBe(key.substring(1).replace(/\//g, path.sep));
        expect(testCache._sanitize(key).safeNamespace).toBe('test/1/'.replace(/\//g, path.sep));
      } else {
    ...
    test('namespaces are distinct', () => {
      ...
      const ns1 = ['a', '1'];
      const ns2 = ['a', '2'];
      return Promise.all([testCache.get({
        key,
        namespace: ns1
      }), testCache.get({
        key,
        ...
        return Promise.all([testCache.put({
          key,
          namespace: ns1,
          data: 'one'
        }), testCache.put({
      ...
      }).then(() => Promise.all([testCache.get({
        key,
        namespace: ns1
      }), testCache.get({
        key,
...
describe('Remote cache specific tests', () => {
  const sandbox = sinon.createSandbox();
  test('non-functional', () => {
    const cache = new _ssrCache.PersistentCache({
      useLocalCache: false,
...
describe('Local cache specific tests', () => {
  ...
  test('Cache cleaned correctly', () => {
    ...
    // never exist
    const fakeDir = fs.mkdtempSync(_ssrCache.PersistentCache._cachePathForPID(999999));
    expect(fs.existsSync(fakeDir)).toBe(true); // Next, create a cache, and make sure it has a directory
    ...
    expect(fs.existsSync(cacheDir)).toBe(true); // Run the cleanup logic. We expect this to clean up
    // the fake directory, but leave the cache's directory.

    return _ssrCache.PersistentCache._cleanupOnEntry().then(() => {
      // Check the results
      expect(fs.existsSync(fakeDir)).toBe(false);
      expect(fs.existsSync(cacheDir)).toBe(true); // Close the cache and check that it cleaned up
