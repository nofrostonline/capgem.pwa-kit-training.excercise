...
const rimraf = require('rimraf'); // Promisified fs functions
...
const ENOENT = 'ENOENT'; // If we're configured to use S3, we require() the AWS sdk on-demand.

let AWS; // mS in one year
...
class PersistentCache {
  ...
  constructor({
    ...
    this._sendMetric = sendMetric; // In order to prevent a very unlikely possibility that the
    ...
    if (useLocalCache) {
      ...
      this._functional = true; // On the first creation of a local cache, we cleanup
      ...
      if (this._functional) {
        ...
        const s3Params = {
          ...
          endpoint: s3Endpoint,
          credentials: accessKeyId ? new AWS.Credentials(accessKeyId, secretAccessKey) :
          /* istanbul ignore next */
          undefined
        };
  ...
  static _cleanupOnEntry() {
    return new Promise((resolve, reject) => {
      ...
      const tmp = os.tmpdir(); // Scan the temporary directory
      ...
      fs.readdir(tmp, (err, entries) => {
        ...
        entries // Test each entry with the regex
        .map(entry => nameRE.exec(entry)) // Filter out any non-matches
        .filter(entry => !!entry) // Check each match
        .forEach(entry => {
  ...
  _sanitize(key, namespace) {
    let safeNamespace; // Use the correct separator to join elements into a path

    const separator = this._local ? path.sep : '/';
    const keyElements = key.split('/'); // safeKey is the last part of generated cache key
    // it is the hashed value

    const safeKey = keyElements.pop(); // For each element of the namespace, convert slashes, whitespace,
    // asterisks to underscore, then join the namespace elements.

    const workingNamespace = []; // Include any prefix as the leading path element. This is only
    // relevant for S3.
    ...
    workingNamespace.push(...keyElements); // eslint-disable-next-line

    const cleanupRE = /[\s\/*\\]/gi; // Build safeNamespace as a path, ending in a path separator, so that
    // it will work as both a local and S3 directory path. Any falsy
    // elements in the namespace Array are ignored..

    safeNamespace = workingNamespace // Remove empty or undefined elements
    .filter(element => !!element) // Clean up element characters
    .map(element => element.replace(cleanupRE, '_').trim()) // Join together as a directory path, ending with a slash
  ...
  _localGetter(key, namespace) {
    const {
      safeKey,
      safeNamespace
    } = this._sanitize(key, namespace);

    const filePath = path.join(this._cachePath, safeNamespace, safeKey);
    return readFileAsync(filePath, this._fileOptions).then(fileData => {
      ...
      const {
        data,
        metadata
      } = cacheData; // Check for expiration.
  ...
  _s3SafeGet(params) {
    return this._s3.getObject(params).promise().catch(err => {
      // We don't expect errors, so we'll log

      /* istanbul ignore next */
      if (err.code !== 'NoSuchKey') {
        console.log(`Unexpected error ${err} from s3.getObject for ${JSON.stringify(params)}`);
      }
  ...
  _remoteGetter(key, namespace) {
    ...
    return Promise.all([this._s3SafeGet(dataGetParams), this._s3SafeGet(metadataGetParams)]).then(([dataResult, metadataResult]) => {
      ...
      const objectMetadata = JSON.parse(metadataResult.Body.toString()); // S3 package does not implement automatic expiration,
  ...
  _internalGet({
    ...
    if (this._functional) {
      return this._local ? this._localGetter(key, namespace) : this._remoteGetter(key, namespace);
    } else {
  ...
  get({
    ...
    const _metricName = entry => {
      return entry.found ? 'ApplicationCacheRetrievalTimeHit' : 'ApplicationCacheRetrievalTimeMiss';
    };

    return this._calculateExecutionMetrics(this._internalGet.bind(this, {
      key,
      namespace
    }), _metricName).then(entry => {
  ...
  _localSetter(key, namespace, data, metadata, expiration, isJSON) {
    ...
    const metaDataAsBuffer = Buffer.from(JSON.stringify({
      expiration,
      isJSON,
      key,
      namespace,
    ...
    const toCache = [metaDataLength, metaDataAsBuffer]; // Track the total length of the buffers; this halves the time needed
    ...
    const fileData = Buffer.concat(toCache, totalLength); // Verify that the full path exists, creating it if
    // it doesn't.
  ...
  _remoteSetter(key, namespace, data, metadata, expiration, isJSON) {
    ...
    return Promise.all([this._s3.putObject(dataPutParams).promise(), this._s3.putObject(metadataPutParams).promise()]);
  }
  /**
  ...
  _internalPut({
    ...
    let isJSON = false; // Serialize anything that isn't a Buffer
    ...
    let workingExpiration = expiration || ONE_YEAR; // If the expiration value is less than DELTA_THRESHOLD,
    // consider it as a delta, otherwise consider it absolute.

    const expirationTimestamp = workingExpiration < PersistentCache.DELTA_THRESHOLD ? Date.now() + workingExpiration : workingExpiration;
    return this._local ? this._localSetter(key, namespace, workingData, metadata, expirationTimestamp, isJSON) : this._remoteSetter(key, namespace, workingData, metadata, expirationTimestamp, isJSON);
  }
  /**
  ...
  put({
    ...
    return this._calculateExecutionMetrics(this._internalPut.bind(this, {
      key,
      namespace,
      data,
      metadata,
  ...
  _remoteDeleter(key, namespace) {
    ...
    return Promise.all([this._s3.deleteObject(dataDeleteParams).promise(), this._s3.deleteObject(metadataDeleteParams).promise()]).catch(err => {
  ...
  delete({
    ...
    return this._local ? this._localDeleter(key, namespace) : this._remoteDeleter(key, namespace);
  }
  ...
  _calculateExecutionMetrics(functionToTimeExecution, metricNameFunc) {
    ...
    return functionToTimeExecution().then(returnObject => {
      this._sendMetric(metricNameFunc(returnObject), new Date() - putTimeStart, 'Milliseconds');
