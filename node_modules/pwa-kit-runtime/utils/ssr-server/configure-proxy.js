 ...
 */
const ALLOWED_CACHING_PROXY_REQUEST_METHODS = ['HEAD', 'GET', 'OPTIONS'];
/**
...
exports.ALLOWED_CACHING_PROXY_REQUEST_METHODS = ALLOWED_CACHING_PROXY_REQUEST_METHODS;
...
const configureProxy = ({
  ...
  const config = {
    ...
    onProxyReq: (proxyRequest, incomingRequest) => {
      ...
      Object.entries(newHeaders).forEach( // setHeader always replaces any current value.
      ([key, value]) => proxyRequest.setHeader(key, value)); // Handle deletion of headers.
    ...
    onProxyRes: (proxyResponse, req) => {
      ...
      const matchedUrl = generalProxyPathRE.exec(req.originalUrl); // Rewrite key headers
  ...
  const proxyFunc = (0, _httpProxyMiddleware.default)(config); // For a standard proxy, we're done
