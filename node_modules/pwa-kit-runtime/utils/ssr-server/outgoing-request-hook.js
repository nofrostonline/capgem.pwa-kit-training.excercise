...
const outgoingRequestHook = (wrapped, options) => {
  return function () {
    ...
    const isLoopback = // Either hostname or host are allowed in the options. The docs
    ...
    workingOptions.headers = workingOptions.headers ? _objectSpread({}, workingOptions.headers) : {}; // Inject the access key.

    workingOptions.headers['x-mobify-access-key'] = accessKey; // Create and add keep-alive agent to options for loop-back connection.
    ...
    if (proxyKeepAliveAgent) {
      ...
      const {
        httpAgent,
        httpsAgent
      } = getKeepAliveAgents(); // Add default agent to global connection reuse.

      workingOptions.agent = workingUrl.startsWith('http:') || ((_workingOptions = workingOptions) === null || _workingOptions === void 0 ? void 0 : _workingOptions.protocol) === 'http:' ? httpAgent : httpsAgent; // `node-fetch` and potentially other libraries add connection: close heaaders
