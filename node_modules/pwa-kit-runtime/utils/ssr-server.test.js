...
beforeEach(() => {
  ...
  console.log = consoleLog; // Suppress test console warns

  consoleWarn = sandbox.spy();
  console.warn = consoleWarn; // Suppress test console errors
...
describe('utils/ssr-server tests', () => {
    ...
    const testCases = [{
      ...
      mobify: {
        ssrParameters: {
          ...
          proxyConfigs: Array.apply( // eslint-disable-line prefer-spread
          null, {
  ...
  describe('detectDeviceType', () => {
    const tests = [// Test all mobile user agents
...
describe('MetricsSender', () => {
  ...
  test('MetricsSender.send', () => {
    ...
    const metrics3 = []; // Add metrics until we get enough to cause multiple batches to
    // be sent.
    ...
    sender._CW = {
      putMetricData: (params, callback) => {
        callCount += 1; // This returns a fake error on the first call, and then
    ...
    return sender.flush().then(() => {
      ...
      expect(sender.queueLength).toBe(0); // Expect that there was a console warning

      expect(consoleWarn.called).toBe(true); // Expect that two batches were sent

      expect(callCount).toBe(2); // Expect that the correct values were sent
  ...
  test('MetricsSender.throttling', () => {
    ...
    sender._CW.putMetricData = sandbox.spy(sender._CW, 'putMetricData'); // Send the params.
...
describe('PerformanceTimer tests', () => {
  let timer;
  beforeEach(() => timer = new _ssrServer.PerformanceTimer(''));
  afterEach(() => {
    var _timer;

    return (_timer = timer) === null || _timer === void 0 ? void 0 : _timer._observer.disconnect();
  });
  test('time() function', () => {
    const func = _sinon.default.stub();
  ...
  test('start() and end()', /*#__PURE__*/_asyncToGenerator(function* () {
    timer.start('test3');
...
describe('wrapResponseWrite', () => {
  test('captures chunks', () => {
    ...
    const chunks = caching.chunks; // String without encoding

    response.write('12345');
    expect(chunks.length).toEqual(1);
    expect(chunks[0].toString()).toEqual('12345');
    expect(write.calledWith('12345'));
    write.reset(); // String with encoding
