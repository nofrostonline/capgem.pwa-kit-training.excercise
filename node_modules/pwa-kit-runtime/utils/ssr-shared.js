 ...
 * @type {string[]}
 */
const validProxyProtocols = ['http', 'https']; // This value is the 'mobify' object from package.json. It should be accessed
// via getPackageMobify() so that test code can update the value to test
// specific functions.

exports.validProxyProtocols = validProxyProtocols;
...
exports.MAX_PROXY_CONFIGS = MAX_PROXY_CONFIGS;
let proxyConfigs = [];
/**
...
const updatePackageMobify = newValue => {
  _packageMobify = newValue || _packageMobify || {}; // Clear and update the proxyConfigs array
  ...
  for (let index0 = 0; index0 < MAX_PROXY_CONFIGS; index0++) {
    ...
    let config;
    const defaultPath = index0 ? `base${index1}` : 'base'; // Support the OLD format of proxy configs.
