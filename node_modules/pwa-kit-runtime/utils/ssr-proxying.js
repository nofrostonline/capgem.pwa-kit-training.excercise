...
class Headers {
  ...
  constructor(headers, format) {
    ...
    this.httpFormat = format === 'http'; // Within this class, headers are represented by an object that maps
...
const monthAbbr = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec']; // Return the given number as a 2-digit string with a leading zero
...
const HOST_AND_PORT_TESTS = [// IPV6 address plus port
{
  type: IPV6,
  regexp: /^\[([a-fA-F0-9:]+)\]:(\d+)$/,
...
const rewriteDomain = (domain, appHostname, targetHost) => {
  ...
  const domainString = domainElements.join('.'); // Does the appHostname include a port number? We need a version
  ...
  const parsedHost = parseHost(appHostname); // If the target host equals or ends with the domainString
  ...
  const targetHostElements = targetHost.split('.'); // Work out how many elements have been removed to form the subdomain

  const strippedOff = targetHostElements.length - domainElements.length; // Strip the same number of elements off the appHost
...
const rewriteProxyResponseHeaders = ({
  ...
  const targetOrigin = `${targetProtocol}://${targetHost}`; // Set the X-Proxy-Request-Url header, if we have the request URL
  ...
  if (statusCode >= 301 && statusCode <= 308) {
    ...
    if (location) {
      ...
      const locUrl = new _url.URL(location, targetOrigin); // If the location header URL is on the targetOrigin, we rewrite it.
      ...
      if (locUrl.protocol === `${targetProtocol}:` && locUrl.host === targetHost) {
        ...
        locUrl.host = appHostname; // Since the proxyPath ends with a slash and locUrl.pathname
...
const rewriteProxyRequestHeaders = ({
  ...
  const workingHeaders = new Headers(_objectSpread({}, headers), headerFormat); // Strip out some specific X-headers

  X_HEADERS_TO_REMOVE.forEach(key => workingHeaders.deleteHeader(key)); // For a caching proxy, apply special header processing
