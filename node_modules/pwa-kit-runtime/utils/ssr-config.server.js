...
const getConfig = () => {
  ...
  const defaultSearchPlaces = SUPPORTED_FILE_TYPES.map(ext => `config/default.${ext}`); // Combined search places.

  const searchPlaces = [...targetSearchPlaces, ...(!isRemote ? localeSearchPlaces : []), ...defaultSearchPlaces, 'package.json'];

  const {
    cosmiconfigSync
  } = require('cosmiconfig'); // Match config files based on the specificity from most to most general.
